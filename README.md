...
- **账单导入** — 支持支付宝 CSV（GBK 编码）和微信支付 XLSX 文件，支持多文件批量预览确认后再导入
  - 导入前摘要：显示总条数、收入/支出金额、重复条数、异常条数
  - 预览确认：支持批量选中有效项、批量改分类、删除误识别记录
  - 重复高亮：按来源订单号优先识别历史重复记录，确认页内直接提示
...
- **自动化测试** — 38 个 pytest 测试用例覆盖解析器、图片识别、API 接口、分类规则与导入确认流程
...
在“导入账单”页面上传支付宝 CSV 或微信支付 XLSX 后，系统会先进入预览确认，而不是直接落库。

- 支持一次上传多份账单文件，系统会为每份文件生成独立预览卡片，方便分别检查和统一确认
- 顶部会展示本次账单的总条数、已选条数、收入/支出汇总、异常条数和重复条数
- 默认自动选中非重复记录，重复记录会保留提示但不会默认导入
- 支持批量选中异常项、批量套用分类、删除不想导入的记录
- 兼容支付宝/微信账单里的 `0.00` 金额记录，不会因为预览校验而中断导入流程
- 确认导入前可以直接修改分类和备注，适合快速整理支付宝/微信账单
...
| 方法 | 路径 | 说明 |
|------|------|------|
| `POST` | `/imports` | 上传并直接导入账单文件 |
| `POST` | `/imports/preview` | 预览支付宝/微信账单文件，返回重复与异常提示 |
| `POST` | `/imports/confirm` | 确认导入预览后的文件记录 |
...
导入时自动检测文件类型。现在支持多文件批量上传、逐份预览后统一确认导入，重复记录会在确认页高亮提示并默认跳过。
